...
echo  "$($PSQL "SELECT t.name AS name FROM games g JOIN teams t ON g.winner_id=t.team_id WHERE year=2014 AND round='Eighth-Final' UNION SELECT t.name AS name FROM games g JOIN teams t ON g.opponent_id=t.team_id WHERE year=2014 AND round='Eighth-Final'")"
